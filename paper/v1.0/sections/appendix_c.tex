...
\end{equation}
derived by separating the aperiodic and periodic components of Eq.~(\ref{eq_12}), fulfill the $t$-$t'$-Schrödinger equation \cite{grifoni98,wackerl20}
...
\end{equation}
by introducing new wave function $\Psi_{n,m}$ for the system with the given total perturbation. If $t\leq t_0$, both solutions of the Schrödinger equations (Eq.~(\ref{eq_c2}) and Eq.~(\ref{eq_c5})) coincide
...
\begin{equation} \label{eq_c7}
  ...
  \ket{\Psi_{n,m}(t,t')},
\end{equation}
and due to time independence, the perturbation in the interaction picture has the same form as Schrödinger picture
...
Now consider a scattering phenomenon from a $t$-$t'$-Floquet state $\ket{\psi_{\beta}(t,t')} $ into a distinct $t$-$t'$-Floquet state $\ket{\Psi_{\alpha}(t,t')}$ that occupied with a constant quansienergy $\varepsilon$:
...
Since we are presenting the the perturbation potential $V(\vb{r})$ by using a group of randomly distributed impurities, consider $N_{imp}$ number of identical single impurity potentials distributed at randomly but fixed positions $\vb{r}_i$. Then scattering potential $V(\vb{r})$ can be identified as the sum of uncorrelated single impurity potentials $\upsilon(\vb{r})$
...
Now using Eq.~(\ref{eq_9}) and Eq.~(\ref{eq_c25}) on Eq.~(\ref{eq_c19}), we obtain (with changing variable $t' \rightarrow t'$)
...
\begin{equation} \label{eq_c29}
  ...
  \end{aligned}
\end{equation}
To avoid the energy exchange from external strong field and electrons in Landau levels, the applied radiation must be a purely dressing field.
Therefore, in this study we assume that the dressing field only can renormalize the the probability of electron scattering inside the exact same Landau energy level $(n_{\alpha} = n_{\beta} =N)$. This transform the Eq.~(\ref{eq_c29}) to
...
\end{equation}
where $J_l(z)$ are Bessel functions of the first kind with $l$-th integer order, and we can re-write the Eq.~(\ref{eq_c32}) as follows
...
\begin{equation} \label{eq_c38}
  ...
  \end{aligned}
\end{equation}
Then describing the square of the delta distribution using following procedure \cite{dini16,kibis14}
...
\end{equation}
and executing the time derivation operation on each matrix element we receive the \textit{transition amplitude matrix} element:
...
An impurity average of white noise potential allows to identify $\expval{|V_{{k'}_x,k_x}|^2} = V_{imp}$ and the inverse scattering time matrix is the sum of all available momentum over the transition probability matrix element
