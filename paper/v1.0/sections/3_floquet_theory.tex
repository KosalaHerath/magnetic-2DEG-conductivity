...
\end{equation}
It is important to notice that these Floquet modes are time-periodic ($T=2\pi/\omega$) functions. At the resonance $\omega = \omega_0$, the energy levels occupies a continuous form and the discrete quasienergies are no longer valid \cite{popov70}. Therefore, in this work we choose the dressing field frequency as $\omega \neq \omega_0$.

Then performing Fourier transform over the confined two-dimentional space $A=L_xL_y$, we obtain the momentum space($k_x,k_y$) representation of Floquet modes
