...
\end{equation}
form this solutions we are able to derive the final solutions $\alpha=(n,m)$ where $n,m \in \mathbb{Z}^{+}$ are two quantum numbers that describe the state of the electron, can be present as
\begin{equation} \label{1.52}
  \begin{aligned}
    \psi_{\alpha}(x,y,t)  = & \frac{1}{\sqrt{L_x}} \chi_n\big(y - \zeta(t)\big) \\
    & \times
      \exp(
